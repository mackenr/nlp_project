...
def basic_clean(text, regexfilter=r"[^a-z0-9\'\s]"):
    ...
    newtext = (
        unicodedata.normalize("NFKD", newtext).encode("ascii", "ignore").decode("utf-8")
    )

    # use re.sub to remove special characters
    newtext = re.sub(rf"{regexfilter}", " ", newtext)
...
def stemmed(text, regexfilter=r"[^a-z0-9\'\s]"):
    ...
    stemfiltered = list(
        filter(
            lambda x: (len(x) > 1 and len(x) < 9 and x.isalpha() == True),
            stemmedlist.split(),
        )
    )
...
def dictlist_super_NLP_comp(
    dictlist, regexfilter=r"[^a-z0-9\'\s]", stop_words_extend_reduce=["'"]
):
    """"""
    ...
    for i in range(0, len(ndictlist)):
        ...
        stopfilteredlemitezed = stopfilter(
            lemmatized, stop_words_extend_reduce=stop_words_extend_reduce
        )
        lemma = {f"lemmatized": stopfilteredlemitezed}
        stem = stemmed(text, regexfilter)
        stopfilteredstem = stopfilter(
            stem, stop_words_extend_reduce=stop_words_extend_reduce
        )
